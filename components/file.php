...
echo '</label>';
echo '<input name="' . htmlentities($name) . '" data-value="' . htmlentities($value) . '"' . (strlen($value) > 0 ? ' data-has-value="true"' : '') . ' id="' . htmlentities($elementID) . '" type="file"' . ($multiple ? ' multiple' : '') . ' accept="' . htmlentities($accept) . '"' . ($maxSize !== '' ? ' data-form-element-data-max-size="' . $maxSize . '"' : '') . '/>';
